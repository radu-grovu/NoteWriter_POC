...
st.caption("Prompts, styles, and test cases persist permanently via Supabase.")
...
def init_supabase_tables():
    try:
        ...
        st.warning("Please create table 'prompt_versions' in Supabase.")
    try:
        supabase.table("test_cases").select("*").limit(1).execute()
    except Exception:
        st.warning("Please create table 'test_cases' in Supabase.")
...
PROMPT_DEFAULTS = {
    ...
    "ap_style": (
        ...
        "- **Impression:** One-line interpretation (etiology, stability, or control).\n"
        "- **Supporting Data:** Key labs/imaging/vitals supporting impression.\n"
        "- **Treatment Plan:** Meds with dose/route/frequency/status (continued/started/changed/holding) + interventions.\n"
        "- **Follow-up & Consults:** Tests, labs, consults, and monitoring steps.\n"
...
Audience: medical experts. Be concise and structured.

Rules:
- Problems in A&P start with hashtag headings (# Problem ...).
- Include Impression, Supporting Data, Treatment Plan, and Follow-up/Consults.
...
with st.expander("▸ Model Settings", expanded=True):
    col = st.columns(3)
    model = col[0].selectbox("Model", ["gpt-4.1", "gpt-5", "gpt-5-pro"], index=0)
...
with st.expander("▸ Inputs / Test Cases", expanded=True):
    ...
    names = sorted(tc_dict.keys())
    c1, c2, c3 = st.columns([2, 1, 1])
    with c1:
        sel_case = st.selectbox("Saved Cases", names, index=0)
    with c2:
        new_case = st.text_input("New Case Name", "")
    with c3:
        if st.button("Load", key="load_tc"):
            case = tc_dict.get(sel_case, {})
            for f, lbl in [("ed_note_input", "ED Note"), ("prior_discharge_input", "Prior Discharge"),
                           ("labs_input", "Labs"), ("imaging_input", "Imaging"),
                           ("med_list_input", "Med List"), ("free_text_input", "Free Text")]:
                st.session_state[f] = case.get(lbl, "")
            st.success(f"Loaded {sel_case}")
    # Save new case
    if st.button("Save New Case", key="save_new_tc"):
        nm = new_case.strip() or f"Case {len(names)+1}"
        data = {
        ...
        save_test_case(nm, data)
        st.success(f"Saved new case: {nm}")
    cdup, cover, cdel = st.columns(3)
    if cdup.button("Duplicate", key="dup_tc"):
        dup = f"{sel_case} (copy)"
        i = 2
        while dup in names: dup = f"{sel_case} (copy {i})"; i += 1
...
with st.expander("▸ Section Instructions / Styles", expanded=False):
    ...
    if b2.button("Duplicate", key="dup_ver"):
        dup = f"{sel_ver} (copy)"
        i = 2
        while dup in vnames: dup = f"{sel_ver} (copy {i})"; i += 1
...
st.download_button("📄 Download assembled_prompt.txt", data=assembled_prompt, file_name="assembled_prompt.txt")
...
if st.button("Generate Note", type="primary", key="gen_btn"):
    ...
    if not result:
        ...
        if not parsed:
            ...
            st.download_button("Download JSON", data=json.dumps(parsed, indent=2, ensure_ascii=False), file_name="note_output.json")
