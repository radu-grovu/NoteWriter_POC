...
# -------------------------------------------------
# Session state setup for persistent sidebar inputs
# -------------------------------------------------
...
def persistent_text_area(key, label, default, **kwargs):
    val = st.text_area(label, value=init_state(key, default), key=key, **kwargs)
...
st.caption("Paste clinical text → get structured note output")
...
with st.sidebar:
    st.header("Settings")
    st.write("API key must be in **Secrets** as `OPENAI_API_KEY`.")
    ...
    hpi_style = persistent_text_area("hpi_style",
        "HPI Style",
    ...
    hp_style = persistent_text_area("hp_style",
    ...
    ap_style = persistent_text_area("ap_style",
        "Assessment & Plan Style",
        "Each problem starts with hashtag heading.\n"
...
def call_model(prompt, model, fallback, max_tokens, temperature):
    ...
    try:
        resp = client.responses.create(
            model=model, input=[{"role": "user", "content": [{"type": "text", "text": prompt}]}],
    ...
    if not result:
        ...
        if not parsed:
            ...
            t1, t2, t3, t4, t5 = st.tabs(
                ["HPI", "A&P", "Physical Exam", "Medication Review", "Source Summary"]
            )
            with t1: st.markdown(parsed["hpi"])
            with t2: st.markdown(parsed["assessment_plan"])
            with t3: st.markdown(parsed["physical_exam"])
            with t4:
                mr = parsed["medication_review"]
                if isinstance(mr, str):
                    st.markdown(mr)
                else:
                    for sec in ["included_medications","excluded_medications","redundancies","interactions","side_effects_relevant"]:
                        st.subheader(sec.replace("_"," ").title())
                        items = mr.get(sec, [])
                        if items:
                            if isinstance(items[0], dict):
                                for i in items:
                                    st.markdown(f"- {json.dumps(i, ensure_ascii=False)}")
                            else:
                                for i in items:
                                    st.markdown(f"- {i}")
                        else:
                    ...
                    st.markdown(mr.get("summary","_None_"))
            with t5: st.markdown(parsed["source_summary"])
            st.download_button("Download JSON", data=json.dumps(parsed, indent=2), file_name="note_output.json")
